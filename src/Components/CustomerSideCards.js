...
export default function CustomerSideCard({itemImage, itemName, itemPrice, itemDeliveryType, itemsRestaurantId }) {

    const History = useHistory()
    const {state, dispatch} = useContext(GlobalContext)
    const makeOrder = async () => {
       await dispatch({type:"ORDER_INFO" , payload: {itemsRestaurantId, itemImage, itemName, itemPrice, itemDeliveryType}})
